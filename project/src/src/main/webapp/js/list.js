...
function setPaging(href, digit){
	...
	if(href==""){
		if(isNaN(digit)){//이전&nbsp;또는 다음 &nbsp;
			gray="gray";  //11,20번 라인 처럼 href속성이 없고 <a>요소의 textnode가 숫자가 아닌 경우
			
		}else{
			active="active"; //12,19번 라인처럼 href 속성이 없고 <a>요소 textnode가 숫자인 경우
		}
	}
				...
				$(data.boardlist).each(
					function(index,item){
						...
						output +="<td><div>" + blank + img
	     				output +='<a href=BookDetailAction.bo?num='+item.board_num + '">'
						...
						$(".pagination").empty(); //페이징 처리 영어 내용 제거
						output ="";
